...
namespace dc
{
    ...
    {        
        ...
        private void OnMessageReveived(long conn_idx, ushort header, ByteArray data)
        {
            ...
            if(!m_app_servers.TryGetValue(conn_idx, out app_server))
            {
                if (packet.header == inner.msg.REQ_LOGIN)
                {
                    if(!m_app_servers.ContainsKey(conn_idx))
                    {
                        ...
                        rep_msg.srv_info.srv_type = eServerType.DB;
            ...
            if(app_server != null)
            {
                if (!app_server.HandleMsg(conn_idx, packet))
                {
                    ...
                    //{
                    //    case inner.msg.APPSERVER_RESLEADED:
        ...
        private void OnGameEvent(GameEvent evt)
        {
            switch (evt.type)
            {
                case EventID.NET_CONNECTED_WORLD_SRV:
                    this.Start(ServerConfig.net_info.port_for_server);
                    break;
        ...
        /// 根据服务器id发
        /// </summary>
        public int Send(ushort sid, PacketBase packet)
        {
            ConnAppProc app_server;
            if(m_srv_servers.TryGetValue(sid, out app_server))
            {
                return Send(app_server.conn_idx, packet);
            }
            else
            {
                PacketPools.Recover(packet);
                Log.Warning("未找到服务器id:" + sid);
                return 0;
            }
        }
        /// <summary>
